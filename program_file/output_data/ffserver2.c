...
if( avio_open_dyn_buf(&ctx->pb) >= 0) {
    ...
    if (c->stream ){
if( !c->post ){
if( c->stream->stream_type == STREAM_TYPE_LIVE)
        current_bandwidth -= c->stream->bandwidth;}}

    /* signal that there is no feed if we are the feeder socket */
    if (c->state == HTTPSTATE_RECEIVE_DATA ){
if( c->stream) {
...
if(
    ...
    case HTTPSTATE_SEND_DATA_TRAILER:
        ...
        if (c->stream->single_frame ){
if( c->data_count > c->cur_frame_bytes ){
if( c->cur_frame_bytes > 0) {
            close_connection(c);
        }}}
        break;
    case HTTPSTATE_RECEIVE_DATA:
...
if( stream->stream_type == STREAM_TYPE_LIVE)
    ...
    if (c->post == 0 ){
if( config.max_bandwidth < current_bandwidth) {
        c->http_error = 503;
        q = c->buffer;
        snprintf(q, c->buffer_size,
                      "HTTP/1.0 503 Server too busy\r\n"
...
if(
            ...
            /* We have timed out */
            c->state = HTTPSTATE_SEND_DATA_TRAILER;}else {
            AVPacket pkt;
        redo:
